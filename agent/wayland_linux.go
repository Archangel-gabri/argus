...
var waylandEncoders = []waylandEncoder{
	{
		...
		build: func(_ [2]int, fps int) []string {
			// cudaupload переносит кадр в память видеокарты до кодирования — без него NVENC
			// получает кадр из системной памяти и копирует его сам, лишний раз.
			return append([]string{"videoconvert", "!", "video/x-raw,format=NV12", "!", "cudaupload"},
				nvencChain(fps)...)
		},
	},
	{
		// Тот же NVENC, но без CUDA-загрузки: на части драйверов путь через cudaupload не
		// договаривается по форматам, а прямой работает.
		name: "nvenc", fps: 60,
		build: func(_ [2]int, fps int) []string {
...
func gstArgs(enc waylandEncoder, st portalStream, w, h, fps int) []string {
	args := []string{
		...
		// ЧАСТОТУ ЗДЕСЬ НЕ ОГРАНИЧИВАЕМ, и это решение, а не упущение. Проверено на живой машине:
		//   • договор о частоте с источником (`video/x-raw,framerate=[1/1,60/1]` сразу за
		//     pipewiresrc) роняет переговоры целиком — «streaming stopped, reason not-negotiated»;
		//   • `videorate` придерживает кадр, чтобы посчитать темп по двум соседним, а портал
		//     отдаёт кадры только при изменениях картинки: на спокойном рабочем столе выходила
		//     задержка в секунды и ноль кадров за время проверки варианта.
		// Поэтому отдаём столько, сколько даёт композитор (на мониторе 100 Гц это ~97 к/с при
		// 5 Мбит/с — NVENC справляется), а решение «столько не нужно» принимает клиент: он
		// считает фактическую частоту сам и просит сбавить, если не успевает.
	}
