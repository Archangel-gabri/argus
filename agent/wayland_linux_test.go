...
func TestGstArgsScaleOnlyWhenNeeded(t *testing.T) {
	...
	// videorate убран сознательно: он придерживает кадр, а источник отдаёт кадры только при
	// изменениях картинки — на спокойном экране это давало задержку в секунды.
	if contains(scaled, "videorate") || contains(native, "videorate") {
		t.Error("videorate вернулся в конвейер — он ломает захват на неподвижном экране")
	}
}
