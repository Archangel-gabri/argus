...
func splitAnnexB(r io.Reader, onAU func(AU)) error {
	...
	for {
		...
		if b == 0x01 && zeros >= 2 {
			...
			// SPS (7), PPS (8) и SEI (6) после последнего VCL-NAL тоже НАЧИНАЮТ access unit
			// (7.4.1.2.3), и без этого условия они уезжали в предыдущий кадр. Живой поток идёт
			// как `[слайсы P][SPS][PPS][слайсы IDR]`: AUD не ставит ни nvh264enc, ни x264enc, а
			// `config-interval=-1` повторяет параметры перед каждым ключевым. Заголовки
			// доставались P-кадру, и вместе с ними — признак «ключевой» (isKey взводится на SPS):
			// клиент, ждущий ключевой кадр после потери, получал P-слайсы без IDR, а настоящий
			// IDR приходил без параметров и декодер не настраивался.
			startsAU := firstSlice || t == 9 || t == 6 || t == 7 || t == 8
			if startsAU && hasVCL {
				flush()
			}
			if isVCL {
