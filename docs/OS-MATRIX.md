...
| Метрики | ✅ | ✅ | ✅ свой зонд на sysctl | ✅ свой зонд на sysctl | ⚠️ busybox, часть полей пуста |
| Порты | ✅ `ss` | ✅ | ✅ `lsof -F` | ✅ `sockstat` | ⚠️ зависит от сборки |
...
- **Порты**: команда выбирает инструмент по `uname -s` — `ss` (Linux), `sockstat -46l` (FreeBSD),
  `lsof` (macOS). Разбор написан для всех трёх, а **формат определяется по самому выводу**, а не по
  тому, какой ОС мы её считаем: ошиблись в определении ОС — получили бы пустой список, который
  выглядит как «портов нет». Если формат не узнан — честная ошибка, а не пустой список.
  На macOS берётся машиночитаемый вывод `lsof -F`: в колоночном имена процессов с пробелами
  («Google Chrome») сдвигают все поля.
