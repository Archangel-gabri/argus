...
> **АРХИВ — снимок интерфейса на 2026-07-01.**
>
> Что уже неверно: экранов не шесть, а пять — Dashboard и Streaming удалены вместе с файлами
> `views/DashboardView.tsx` и `views/StreamingView.tsx`, на которые ссылается текст ниже;
> «Unlock Nexus One» — старое имя. Карта текущего интерфейса пересобирается в Фазе 4 мандата.
...
>
> **B2 применён (2026-07-11, ветка `feat/argus-b2`):** карточка сервера v2 — «сцена» с
> изо-портретом по роли (`lib/illustrations.ts`), статус-кольцом, флагом-кружком, бейджем
> хостера, кнопками SSH/Panel; спарклайн переехал в drawer. Появился **детальный drawer**
> устройства (клик по сцене; табы Обзор/Terminal/Файлы/Порты/Метрики) — модалки
> Terminal/SFTP/Forwards (§6.3–6.5) УДАЛЕНЫ, их логика живёт в `components/panes/*`,
