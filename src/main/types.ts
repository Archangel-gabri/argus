...
export interface Subscription {
  ...
  /**
   * Устройство, за которое платят этой подпиской.
   *
   * Без связи одна и та же трата попадала в месячный расход ДВАЖДЫ: сервер заведён и в парке
   * (с ценой), и подпиской — приложение складывало обе строки и не могло знать, что это одно.
   */
...
export interface SubscriptionInput {
  ...
  renewalDay?: number | null
  /** Устройство, за которое платят: связь убирает двойной счёт одной и той же траты. */
  deviceId?: string | null
  currency?: Currency
  period?: 'mo' | 'yr'
  nextRenewal?: string | null
