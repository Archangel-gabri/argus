...
describe('доказанный отказ не подменяется сроком', () => {
  it('401 в заголовках при молчащем теле даёт «ключ неверен», а не «таймаут»', async () => {
    // Контрпример от адверсарной проверки: код ответа — уже доказанный факт. Пока вердикт
    // ставился после чтения тела, ответ «401 сразу, тело молчит» через десять секунд
    // превращался в «сервис не ответил»: владелец видел «проверка не удалась» вместо «ключ не
    // принят» и шёл чинить связь вместо ключа.
    vi.useFakeTimers()
    vi.stubGlobal(
      'fetch',
    ...
    const verdict = await checkAccount('openrouter', 'sk-протухший')
