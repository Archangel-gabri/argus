...
import { execOnce, resolveConn } from '../remote/ssh'
import { whichOs, osReachable, unreachableReason } from '../devices/pc'
import { getAgentToken, setAgentToken, getAgentCert, setAgentCert } from '../vault/vault'
import { buildForgetCommand, decideForgetOutcome, type ForgetOutcome } from './agent-forget'
import { remoteFamily, remoteArch } from '../devices/os-family'
import { hostForUrl } from '../remote/ip-privacy'
...
async function writeTokenFile(
  ...
  const { sftpOpen, sftpWriteFile, sftpClose } = await import('../remote/sftp')
...
async function uploadFile(
  ...
  const { sftpOpen, sftpPutFile, sftpClose } = await import('../remote/sftp')
  const s = await sftpOpen(deviceId)
  if (!s.ok || !s.sessionId) return { ok: false, error: s.error || 'sftp не открылся' }
  try {
