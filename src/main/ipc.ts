...
import * as ssh from './remote/ssh'
import * as sftp from './remote/sftp'
import * as forward from './remote/forward'
import { parseSshConfig, type ParsedHost } from './remote/sshconfig'
import { discoverTailscale } from './devices/discovery'
import { walletBalance } from './finance/onchain'
import { checkAccount, keyEndpointBase } from './ai/ai'
import * as aiPrices from './ai/ai-prices'
import { seedPricesIfEmpty } from './ai/ai-prices'
import { seedAiAccess } from './ai/ai-seed'
import { seedSubscriptions } from './finance/subs-seed'
import { seedFinanceAccounts } from './finance/finance-seed'
import { linkPaidDevices } from './ai/spend-link'
import { brandIcon } from './support/brand-icon'
import { migrateToAccounts } from './ai/ai-accounts-migrate'
import { pruneUnverifiedAccounts } from './ai/ai-accounts-prune'
import { readLogins, type BrowserLogin } from './security/browser-passwords'
import { fetchModels } from './ai/ai-models'
import { fetchQuota } from './ai/ai-quota'
import { exchangeOf, fetchExchangeBalance } from './finance/exchanges'
import { fetchTinvestPortfolio } from './finance/tinvest'
import { fetchTbankBalance } from './finance/tbank'
...
import {
  ...
  setLoginListener,
  type BankId
} from './finance/bank-session'
import { collectUsage } from './ai/ai-usage'
import * as pc from './devices/pc'
import * as ports from './devices/ports'
import * as watchdog from './watchdog'
import * as hardware from './devices/hardware'
import * as screen from './screen/screen'
import * as agent from './screen/agent'
import { ipLookup } from './remote/net'
import { fleetReach } from './devices/liveness'
 ...
 */
setLoginListener(() => {
  // Сообщаем о самом факте входа НЕЗАВИСИМО от того, обновился ли остаток. Раньше событие шло
  // только при `updated > 0`, и у счёта, заведённого руками (а так владелец их и заводит),
  // после входа в кабинет на экране не менялось ничего: кнопка по-прежнему звала «войти»,
  // остаток не появлялся, ошибки не было. Вход выглядел не сработавшим.
  announceAiUpdated('bank-session')
...
function afterUnlock(): void {
  // Каждый шаг отвечает за себя.
  //
  // Раньше все пятеро стояли под одним `try` с пустым `catch`: одна кривая строка в файле
  // засева (неизвестная валюта, сумма строкой) роняла первый шаг — и молча отменяла остальные.
  // Не заводились счета, не выполнялась миграция каналов, не убирались неподтверждённые
  // аккаунты, и так на каждом входе, без единого следа. Заметить можно было только по
  // отсутствию данных.
  const step = (name: string, run: () => void): void => {
    try {
      run()
    } catch (e) {
      // В интерфейс это не поднять — засев идёт до первого кадра, — но в журнале причина
      // обязана быть: молчаливый отказ здесь дороже любого другого.
      console.error(`[seed] шаг «${name}» не выполнен:`, (e as Error).message)
    }
  }

  step('каталог цен', seedPricesIfEmpty)
  step('реестр ИИ-доступов', seedAiAccess)
  ...
  step('слияние каналов', migrateToAccounts)
...
async function refreshExchangeBalances(): Promise<{ updated: number; failed: number; issues: Array<{ accountId: string; error: string }> }> {
  ...
  for (const account of vault.listFinanceAccounts()) {
    ...
    // `source: 'api'`, но этот признак ставится только правкой остатка по ключу — у счёта,
    // заведённого руками, его нет никогда. Получался замкнутый круг: вход есть, а читать
    // некому. Само по себе название «Т-Банк» ничего не включает: нужен живой вход, который
    // делает человек, и без него `hasSession` вернёт ложь.
    if (bankOf(account) === 'tbank') {
...
export function registerIpc(): void {
  ...
  ipcMain.handle('accounts:delete', (_e, id: unknown) => {
    // Отвечаем по факту, а не «ок» всегда: иначе исчезнувшая мимо интерфейса запись и успешное
    // удаление выглядят одинаково.
    const removed = vault.deleteFinanceAccount(asString(id))
    return removed ? { ok: true } : { ok: false, error: 'Счёт не найден' }
  })
