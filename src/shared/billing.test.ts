...
describe('подпись срока у автосписаний', () => {
  it('прошедшая дата автосписания — не «просрочено»', () => {
    // Деньги списывает провайдер, а дата в Argus — заметка владельца. На следующий день после
    // успешного автоплатежа приложение писало красным «просрочено 1 дн.» — ложная тревога,
    // и таких набиралось столько, что владелец перестал верить красному цвету вообще.
    expect(renewalLabel(-1, false)).toBe('дата устарела')
    expect(renewalLabel(-30, false)).toBe('дата устарела')
  })

  it('у ручного продления просрочка остаётся просрочкой', () => {
    // Здесь никто не заплатил, и это правда тревога.
    expect(renewalLabel(-1, true)).toBe('просрочено 1 дн.')
  })

  it('будущее и сегодня подписаны одинаково для обоих', () => {
    expect(renewalLabel(0, false)).toBe('сегодня')
    expect(renewalLabel(3, false)).toBe('через 3 дн.')
    expect(renewalLabel(3, true)).toBe('через 3 дн.')
  })
})
