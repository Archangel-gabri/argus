...
export function ScreenPane({ device }: { device: DeviceDTO }): React.JSX.Element {
  ...
  const open = async (): Promise<void> => {
    if (!api) return
    // Открытие идёт секунды, и нетерпеливый повтор — обычное дело. Кнопка на это время
    // заблокирована, а Enter в поле пароля нет: два нажатия давали два окна экрана на одно
    // устройство, где клиенты дерутся за поток и картинка чернеет. Человек видел два окна
    // «подключено» с чёрным полотном и не понимал, что сломал сам.
    if (opening) return
    ...
    if (!r.ok) {
      ...
      // Пароль учётной записи ОС стираем и при отказе тоже. Раньше очистка стояла ниже этого
      // возврата: после отбитой попытки (опечатка, NLA) он оставался и в состоянии, и в
      // значении поля — до закрытия вкладки.
      setPassword('')
      return
    }
