...
export function SftpBrowser(): React.JSX.Element | null {
  const target = useUI((s) => s.sftp)
  const close = useUI((s) => s.closeSftp)
  ...
  useEffect(() => {
    ...
    void (async () => {
      setLoading(true)
      setError(null)
      const r = await api.sftp.open(target.id)
      if (disposed) return
      if (!r.ok || !r.sessionId) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onMouseDown={close}>
      <div
        className="flex h-[80vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl"
      ...
      >
        <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <span className="font-mono text-xs text-slate-500">SFTP</span>
          </div>
          <button
            onClick={close}
