    ...
    try {
      ...
      await Promise.all(
        eligible.map(async (d) => {
          ...
          //
          // Но это работает только там, где живость ВООБЩЕ может дать ответ. За бастионом
          // прямая TCP-проба недостижима по определению, и живость честно отвечает «не знаю»,
          // не трогая статус. Пропуская такие хосты, приложение запирало их в «Выключен»
          // навсегда: оживить статус было нечем, а перезапуск восстанавливал его из снимка.
          // Единственный источник правды о них — как раз полный опрос, который умеет ходить
