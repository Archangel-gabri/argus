...
export function approxMoney(amountUsd: number, currencies: Iterable<string>): string {
  ...
  //
  // Но округляем ТОЛЬКО то, что от округления не исчезает. Внешний `Math.round` стоял здесь
  // безусловно — и любая сумма меньше полудоллара схлопывалась в «$0»: рублёвый остаток 30 ₽
  // ($0.38) в плитке «Известно» и годовая подписка 300 ₽ ($0.32/мес) читались как «денег нет»
  // и «бесплатно». Это ровно тот запрет, что записан в правилах проекта: перед `money` внешнего
  // округления быть не должно — оно уже умеет показывать «меньше цента, но не ноль».
  if (!converted) return money(amountUsd)
  const whole = Math.round(amountUsd)
  return `≈ ${money(whole === 0 && amountUsd > 0 ? amountUsd : whole)}`
}
