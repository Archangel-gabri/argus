...
/**
 * Цвета категорий для диаграмм.
 *
 * Ни один не совпадает с акцентом приложения (`#f59e0b`). Раньше «Инфраструктура» красилась
 * ровно им — тем же цветом, которым подписана кнопка главного действия и просроченный платёж.
 * Из-за этого сектор диаграммы читался как «требует внимания», хотя означал только вид траты:
 * цвет в приложении занят делом, и одалживать его категории нельзя.
 *
 * Оттенки разведены по тону не меньше чем на двадцать пять градусов: прежние «Инфраструктура»
 * и «Хостинг» были двумя оттенками синего и на диаграмме сливались в один сектор, а понять,
 * где кончается один и начинается другой, было нельзя.
 */
...
export const catColor = (c: string): string => CAT_COLOR[c] ?? '#8a8580'
